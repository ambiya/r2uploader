      ...
      <tbody>
        ...
        <tr>
          ...
          <td style="text-align:right;">
            <button class="btn btn-secondary" onclick="editUser(<?= $user['id'] ?>, '<?= htmlspecialchars(addslashes($user['username']), ENT_QUOTES, 'UTF-8') ?>', '<?= htmlspecialchars($user['role']) ?>')" style="padding:0.4rem 0.65rem;" title="<?= __('title_edit') ?>">
            ...
            <form method="POST" action="/?action=user_delete" style="display:inline;" onsubmit="return confirm('<?= htmlspecialchars(addslashes(__('confirm_delete_user', ['username' => $user['username']])), ENT_QUOTES, 'UTF-8') ?>');">
              <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrfToken) ?>">
              <input type="hidden" name="user_id" value="<?= $user['id'] ?>">
              <button type="submit" class="btn btn-danger" style="padding:0.4rem 0.65rem;" title="<?= __('title_delete') ?>">
